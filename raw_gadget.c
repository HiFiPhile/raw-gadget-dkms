...
#else
#define USB_RAW_IO_LENGTH_MAX PAGE_SIZE
#endif
...
#else
/*
 * USB_RAW_EVENT_{SUSPEND,RESUME,RESET,DISCONNECT} were added in v6.7.
 * Keep the callbacks as no-ops with older UAPI headers.
 */
static void gadget_disconnect(struct usb_gadget *gadget) { }
static void gadget_suspend(struct usb_gadget *gadget) { }
static void gadget_resume(struct usb_gadget *gadget) { }
...
static int raw_ioctl_run(struct raw_dev *dev, unsigned long value)
{
	...
	if (ret) {
		dev_err(dev->dev,
			"fail, gadget driver registration returned %d\n", ret);
		dev->state = STATE_DEV_FAILED;
		goto out_unlock;
	}
